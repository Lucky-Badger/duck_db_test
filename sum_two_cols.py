...
relation_result2 = (
    ...
    .select(
        f"r1.*, \
        {((col('r1.amount1') + col('r2.amount2')).cast(t_cast(DuckDBPyType,'DECIMAL(38,8)')))} AS calc_col" # Alias not working as intended so just rename , have to cast to make my py happy 
    )
)
